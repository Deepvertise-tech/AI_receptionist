...
async function performAction(action, fields, call, req) {
  switch (action) {
    case 'start_message':
      ...
      break;
    case 'send_message': {
      // MESSAGE pipeline ONLY — original simple format with context
...
app.post('/voice/inbound', (req, res) => {
  ...
  const welcome = `Hi! Welcome to ${BUSINESS.name}. How can I help you today? You can order food, book a table, or leave a message. What would you like to do?`;
...
app.post('/voice/handle', async (req, res) => {
  ...
  // Message capture mode (no LLM) — MESSAGE pipeline
  ...
  if (plan.pipeline_done === true) {
    if ((plan.pipeline === 'order' || locked === 'order') && (ctx.computed_total != null)) {
      ...
      // ORDER pipeline ONLY — send your specified email format once
      if (!ctx.order_email_sent) {
        const name = (ctx.name || 'Unknown').trim();
        const contact = (ctx.phone || callerNumber || 'Unknown').trim();
        const item = ctx.order_item || plan.fields?.order_item || 'order';
        const qty = ctx.order_qty ?? plan.fields?.order_qty ?? 1;
        const messageLine = `Order confirmed — ${qty} × ${item}. Total ${ctx.computed_total.toFixed(2)} euros.`;

        const subject = 'New Message from a Customer';
        const text =
`Message:
